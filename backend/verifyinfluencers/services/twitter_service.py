...
def fetch_tweets(username, max_results=10):
    ...
    try:
        ...
        user = client.get_user(username='elonmusk', user_auth=False)
    ...
    if user.data:
        ...
        name = user.data.name
        try:
            followers = client.get_follower_ids(user_id =user_id)
            num_followers = len(followers["ids"])
        except (tweepy.TooManyRequests, tweepy.BadRequest):
            num_followers = 0
